...
function hasPermission (authority, permissions) {
  ...
  return required === undefined || required === '*' || (permissions && permissions.findIndex(item => item === required || item.id === required) !== -1)
}
...
function hasRole (authority, roles) {
  ...
  return required === undefined || authority === '*' || hasAnyRole(required, roles)
}
